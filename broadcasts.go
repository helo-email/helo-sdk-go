...
func (s *BroadcastsService) ListFailures(ctx context.Context, id string) (*PaginatedResponseOfBroadcastFailure, error) {
	out := new(PaginatedResponseOfBroadcastFailure)
	if err := s.client.request(ctx, "GET", fmt.Sprintf("/broadcasts/%v/failures", id), out); err != nil {
		return nil, err
	}
	return out, nil
}
...
func (s *BroadcastsService) ListSuppressions(ctx context.Context, id string) (*PaginatedResponseOfBroadcastSuppression, error) {
	out := new(PaginatedResponseOfBroadcastSuppression)
	if err := s.client.request(ctx, "GET", fmt.Sprintf("/broadcasts/%v/suppressions", id), out); err != nil {
		return nil, err
	}
	return out, nil
}
